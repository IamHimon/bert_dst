...
from baseline.bert import modeling, optimization
from baseline.bert.modeling import BertModel
from baseline.data_utils import *
from baseline.extract_schema_embedding import model_fn_builder, SchemaEmbeddingGenerator
from baseline.pred_utils import write_predictions_to_file

from baseline.data_utils import STATUS_ACTIVE
...
flags.DEFINE_string("bert_ckpt_dir", "F:\projects\dst\multi_cased_L-12_H-768_A-12",
...
flags.DEFINE_enum("task_name", "dstc8_single_domain", FILE_RANGES.keys(),
                  "The name of the task to train.")

flags.DEFINE_string(
    "dstc8_data_dir", "F:\projects\dst\dstc8-schema-guided-dialogue",
...
def _file_based_input_fn_builder(input_dial_file, schema_embedding_file,
    ...
    max_num_cat_slot = MAX_NUM_CAT_SLOT
    max_num_noncat_slot = MAX_NUM_NONCAT_SLOT
    max_num_total_slot = max_num_cat_slot + max_num_noncat_slot
    max_num_intent = MAX_NUM_INTENT
...
class SchemaGuidedDST(object):
    ...
    def define_loss(self, features, outputs):
        ...
        cat_loss_weight = tf.cast(
            tf.equal(cat_slot_status_labels, STATUS_ACTIVE), tf.float32)
        ...
        noncat_loss_weight = tf.cast(
            tf.equal(noncat_slot_status_labels, STATUS_ACTIVE),
            tf.float32)
...
def _model_fn_builder(bert_config, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            output_spec = tensorflow.contrib.tpu.TPUEstimatorSpec(
                mode=mode,
                loss=total_loss,
                train_op=train_op,
...
def _create_schema_embeddings(bert_config, schema_embedding_file):
    ...
    is_per_host = tensorflow.contrib.tpu.InputPipelineConfig.PER_HOST_V2
    schema_emb_run_config = tensorflow.contrib.tpu.RunConfig(
        master=FLAGS.master,
    ...
    schema_json_path = os.path.join(FLAGS.dstc8_data_dir, FLAGS.dataset_split,
                                    "schema.json")
    schemas = Schema(schema_json_path)
    ...
    schema_emb_model_fn = model_fn_builder(
    ...
    # or GPU.
    schema_emb_estimator = tensorflow.contrib.tpu.TPUEstimator(
        use_tpu=FLAGS.use_tpu,
    ...
    emb_generator = SchemaEmbeddingGenerator(
...
def main(_):
    ...
    if task_name not in FILE_RANGES:
    ...
    if not tf.io.gfile.exists(dial_file) or FLAGS.overwrite_dial_file:
        tf.logging.info("Start generating the dialogue examples.")
        _create_dialog_examples(processor, dial_file)
        tf.logging.info("Finish generating the dialogue examples.")
    ...
    if (not tf.io.gfile.exists(schema_embedding_file) or
        ...
        tf.logging.info("Start generating the schema embeddings.")
        _create_schema_embeddings(bert_config, schema_embedding_file)
        tf.logging.info("Finish generating the schema embeddings.")
    ...
    if FLAGS.use_tpu and FLAGS.tpu_name:
        tpu_cluster_resolver = tensorflow.contrib.cluster_resolver.TPUClusterResolver(
    ...
    if FLAGS.run_mode == "train":
        ...
        tf.logging.info("  Num dial examples = %d", num_train_examples)
        ...
        tf.logging.info("  Num actual examples = %d",
        ...
        input_json_files = [
            ...
            for fid in FILE_RANGES[FLAGS.task_name][FLAGS.dataset_split]
        ]
        ...
        for ckpt_num in ckpt_nums:
            ...
            write_predictions_to_file(predictions, input_json_files,
